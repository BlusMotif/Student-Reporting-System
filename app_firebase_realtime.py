...
def load_logged_in_user():
    ...
    if username is None:
        ...
        user = firebase_realtime_db.get_user_by_username(username)
        if user is None:
            g.user = None
        else:
...
def register():
    if request.method == 'POST':
        ...
        existing_user = firebase_realtime_db.get_user_by_username(username)
        if existing_user:
            flash('Username already exists. Please choose a different one.', 'error')
            return render_template('register.html')
        
        # Create new user
        user_id, message = firebase_realtime_db.create_user(username, password, 'student')
        if user_id:
...
def login():
    if request.method == 'POST':
        ...
        user = firebase_realtime_db.verify_password(username, password)
        if user:
...
def dashboard():
    ...
    if g.user['role'] == 'admin' or g.user['role'] == 'subadmin':
        # Admin dashboard
        all_issues = firebase_realtime_db.get_all_issues()
        pending_issues = firebase_realtime_db.get_issues_by_status('pending')
        in_progress_issues = firebase_realtime_db.get_issues_by_status('in_progress')
        resolved_issues = firebase_realtime_db.get_issues_by_status('resolved')
        
        # Get user info for each issue
        for issue in all_issues:
            student = firebase_realtime_db.get_user_by_id(issue['student_id'])
        ...
        # Student dashboard
        student_issues = firebase_realtime_db.get_issues_by_student(g.user['id'])
...
def submit_issue():
    ...
    if request.method == 'POST':
        ...
        issue_id, result_message = firebase_realtime_db.create_issue(g.user['id'], subject, category, message)
        if issue_id:
...
def view_issue(issue_id):
    ...
    issue = firebase_realtime_db.get_issue_by_id(issue_id)
    if not issue:
...
def update_issue(issue_id):
    ...
    success, message = firebase_realtime_db.update_issue_status(issue_id, status, response)
    if success:
...
def list_users():
    ...
    role_counts = firebase_realtime_db.get_user_count_by_role()
...
def profile():
    ...
    user_details = firebase_realtime_db.get_user_by_id(g.user['id'])
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        user = firebase_realtime_db.verify_password(g.user['username'], current_password)
        if not user:
            flash('Current password is incorrect.', 'error')
            return render_template('change_password.html')
        
        # Update password
        success, message = firebase_realtime_db.update_user_password(g.user['username'], new_password)
        if success:
...
def statistics():
    ...
    user_stats = firebase_realtime_db.get_user_count_by_role()
    issue_stats = firebase_realtime_db.get_issue_count_by_status()
